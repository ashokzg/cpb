...
class MCPlanner(object):
    ...
    def perturb_shot(self, table_state):
        mean = (0,0)
        sd = 0.05
        cov = [[sd**2, 0],[0, sd**2]]
        no_of_random_tables = 10
        random_table_states = []
        for t in range(no_of_random_tables):
            random_table_states.append(copy.deepcopy(table_state))            
            for ball in random_table_states[t].balls:
                tx, ty = numpy.random.multivariate_normal(mean, cov, 1)[0]
                ball.point.point.x += tx
                ball.point.point.y += ty
        return random_table_states
